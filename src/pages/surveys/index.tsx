...
import { getSurveys, deleteSurveyById } from 'apiSdk/surveys';
import { SurveyInterface } from 'interfaces/survey';

type ColumnType = ColumnDef<SurveyInterface, unknown>;
...
export function SurveyListPage(props: SurveyListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getSurveys({
        relations: ['organization', 'question.count', 'response.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<SurveyInterface>>(
    () => `/surveys?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: SurveyInterface) => {
    if (hasAccess('survey', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/surveys/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/surveys/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('survey', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/surveys/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('survey', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="survey" />
        </Flex>

        {hasAccess('survey', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/surveys/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
