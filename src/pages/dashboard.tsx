...
import { SurveyListPage as ShowCaseList } from 'pages/surveys';

function DashboardPage() {
  const { session } = useSession();

  const surveyStat = useSWR<PaginatedInterface<SurveyInterface>>(
    () => `/surveys`,
    () => getSurveys({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Surveys',
      result: surveyStat?.data?.totalCount,
      isLoading: surveyStat.isLoading,
    },
    {
      title: 'Questions',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`surveys`} passHref legacyBehavior>
                  <Button
